...
class StressTestRunner {
    constructor() {
        this.concurrency = parseInt(process.argv.find(arg => arg.startsWith('--concurrency='))?.split('=')[1]) || 50;
        this.duration = parseInt(process.argv.find(arg => arg.startsWith('--duration='))?.split('=')[1]) || 60;
    ...
    async runStressTest() {
        ...
        const progressBar = new cliProgress.SingleBar({
            format: 'Progress |' + chalk.cyan('{bar}') + '| {percentage}% | {value}/{total} requests | ETA: {eta}s',
            barCompleteChar: '\u2588',
            barIncompleteChar: '\u2591',
            hideCursor: true,
        });
    ...
    generateReport() {
        ...
        console.log(`  Successful: ${chalk.green(this.metrics.successfulRequests)}`);
        console.log(`  Failed: ${this.metrics.failedRequests > 0 ? chalk.red(this.metrics.failedRequests) : chalk.green('0')}`);
        ...
        console.log(`  Mean: ${stats.mean < 100 ? chalk.green(stats.mean) : chalk.yellow(stats.mean)}`);
        console.log(`  Median: ${stats.median < 100 ? chalk.green(stats.median) : chalk.yellow(stats.median)}`);
        console.log(`  P95: ${stats.p95 < 200 ? chalk.green(stats.p95) : chalk.yellow(stats.p95)}`);
        console.log(`  P99: ${stats.p99 < 500 ? chalk.green(stats.p99) : chalk.yellow(stats.p99)}`);
        console.log(`  Min: ${chalk.green(stats.min)}`);
        console.log(`  Max: ${stats.max > 1000 ? chalk.red(stats.max) : chalk.yellow(stats.max)}\n`);
        ...
        if (this.metrics.queueDepths.length > 0) {
            ...
            for (const queue of queues) {
                const depths = this.metrics.queueDepths.map(d => d[queue]).filter(d => d >= 0);
                if (depths.length > 0) {
        ...
        if (throughput >= 100) {
            ...
            assessments.push(chalk.yellow(`⚠ Moderate throughput (${throughput} req/s)`));
        } else {
