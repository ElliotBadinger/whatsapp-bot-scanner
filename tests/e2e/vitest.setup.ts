  ...
  default: class RedisMock {
    ...
    async keys(pattern: string): Promise<string[]> {
      ...
      if (pattern.endsWith('*')) {
        ...
        return [...new Set([...this.strings.keys(), ...this.lists.keys()])].filter((k) =>
          k.startsWith(prefix),
        );
      }
      return [...new Set([...this.strings.keys(), ...this.lists.keys()])].filter((k) => k === pattern);
    }
...
process.env.IDENTIFIER_HASH_SECRET = process.env.IDENTIFIER_HASH_SECRET || 'e2e-test-secret';
