...
describe("End-to-End System Flow", () => {
  ...
  it("should scan a URL and return a verdict", async () => {
    ...
    while (status === "pending" && attempts < maxAttempts) {
      ...
      const { statusCode: pollStatus, body: pollBody } = await request(
        `${CONTROL_PLANE_URL}/api/v1/scan/${scanId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${API_TOKEN}`,
      ...
      const pollResult = (await pollBody.json()) as {
        status: string;
        verdict?: string;
      };
