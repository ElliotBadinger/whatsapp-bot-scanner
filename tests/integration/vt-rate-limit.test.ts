...
describe('VirusTotal quota handling', () => {
  it('throttles submissions above the 4 req/min window', async () => {
    ...
    fetchMock.mockImplementation(async (input: any) => {
      ...
      if (url.includes('/api/v3/urls')) {
        ...
        return {
          status: 200,
          json: async () => ({ data: { id: `analysis-${submissionTimestamps.length}` } }),
        } as any;
      }
      return {
        status: 200,
    ...
    const analyses = Array.from({ length: 5 }, (_, idx) => vtAnalyzeUrl(`http://example.test/${idx}`));
  ...
  it('throws QuotaExceededError and flips quota gauge on HTTP 429', async () => {
    ...
    expect(metrics).toMatch(/wbscanner_api_quota_depleted_total\{service="virustotal"\} 1/);
  });
});
