...
describe('WhoisXML quota enforcement', () => {
  it('disables lookups when monthly quota reached', async () => {
    ...
    vi.mocked(undici.request).mockResolvedValue({
      statusCode: 200,
      body: { json: async () => ({ WhoisRecord: { domainName: 'example.test', createdDate: '2020-01-01' } }) },
    } as any);

    const { whoisXmlLookup, QuotaExceededError, register } = await import('@wbscanner/shared');

    const first = await whoisXmlLookup('example.test');
    expect(first.record?.domainName).toBe('example.test');

    await expect(whoisXmlLookup('example.test')).rejects.toBeInstanceOf(QuotaExceededError);
