...
vi.mock("url-expand", () => {
  const legacyWrapper = (
    shortUrl: string,
    callback: (error: unknown, expanded?: string | null) => void,
...
describe("Shortener fallback chain (legacy url-expand API)", () => {
  it("returns original URL when Unshorten and url-expand fail", async () => {
    ...
    legacyExpandMock.mockImplementation(
      (_shortUrl: string, callback: (error: Error) => void) => {
  ...
  it("uses url-expand when Unshorten fails", async () => {
    ...
    legacyExpandMock.mockImplementation(
      (
        _shortUrl: string,
        callback: (error: null, expanded: string) => void,
      ) => {
...
describe("Shortener fallback chain (modern url-expand API)", () => {
  ...
  it("uses url-expand modern API when available", async () => {
    ...
    modernExpandMock.mockImplementation(
      async (shortUrl: string, options: any) => {
    ...
    legacyExpandMock.mockImplementation(() => {
      throw new Error(
        "legacy API should not be used when modern expand exists",
      );
    });
  ...
  it("flags responses above the content-length cap", async () => {
    ...
    modernExpandMock.mockImplementation((_shortUrl: string, options: any) =>
      options.fetch("http://bit.ly/oversized"),
    );
    legacyExpandMock.mockImplementation(
      (_shortUrl: string, callback: (error: Error) => void) => {
  ...
  it("reports timeouts from the direct fetch fallback", async () => {
    ...
    modernExpandMock.mockImplementation((_shortUrl: string, options: any) =>
      options.fetch("http://bit.ly/timeout"),
    );
    legacyExpandMock.mockImplementation(
      (_shortUrl: string, callback: (error: Error) => void) => {
        callback(new Error("library failure"));
