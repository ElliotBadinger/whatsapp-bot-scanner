...
describe('Circuit breaker transitions', () => {
  it('transitions through open, half-open, and closed states', async () => {
    ...
    expect(transitions).toContainEqual({ to: CircuitState.OPEN, from: CircuitState.CLOSED });
    ...
    expect(transitions).toContainEqual({ to: CircuitState.HALF_OPEN, from: CircuitState.OPEN });
    expect(transitions).toContainEqual({ to: CircuitState.CLOSED, from: CircuitState.HALF_OPEN });
