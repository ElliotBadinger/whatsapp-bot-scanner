...
const libPath = path.resolve(__dirname, '../.github/scripts/charliehelps-auto-lib.mjs');

describe('charliehelps-auto helpers', () => {
  test('detects working messages', async () => {
    const lib = await import(libPath);
    expect(lib.isWorkingMessage('I’m working on this now and can’t be interrupted or see replies.')).toBe(true);
    expect(lib.isWorkingMessage('Starting a review; I won’t see replies while I’m working.')).toBe(true);
    expect(lib.isWorkingMessage('General update without restrictions.')).toBe(false);
  });
