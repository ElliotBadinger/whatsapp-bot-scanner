...
describe("DependencyManager Tests", () => {
  ...
  describe("Docker Detection", () => {
    it("should detect Docker when available", async () => {
      const mockExeca = vi
        .fn()
        .mockResolvedValue({ stdout: "Docker version 20.10.7" });
      vi.spyOn(execa, "execa").mockImplementation(mockExeca);
...
GSB_API_KEY=old_gsb_key
      ...
      expect(updatedContent).toContain(
        "new_key_12345678901234567890123456789012",
      );
...
describe("Error Handling Tests", () => {
  ...
  describe("Error Handling Function", () => {
    it("should handle DependencyError correctly", () => {
      ...
      expect(
        messages.some(
          (m) => m.type === "error" && m.message.includes("Dependency issue"),
        ),
      ).toBe(true);
    });
    ...
    it("should handle ConfigurationError correctly", () => {
      ...
      expect(
        messages.some(
          (m) =>
            m.type === "error" && m.message.includes("Configuration problem"),
        ),
      ).toBe(true);
    });
    ...
    it("should handle DockerError correctly", () => {
      ...
      expect(
        messages.some(
          (m) => m.type === "error" && m.message.includes("Docker problem"),
        ),
      ).toBe(true);
    });
    ...
    it("should handle PairingError correctly", () => {
      ...
      expect(
        messages.some(
          (m) => m.type === "error" && m.message.includes("Pairing problem"),
        ),
      ).toBe(true);
    });
