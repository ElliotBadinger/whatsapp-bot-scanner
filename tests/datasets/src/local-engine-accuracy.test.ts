...
async function measureAccuracy(
  localThreatDb: LocalThreatDatabase,
  rows: LabeledUrl[],
) {
  let correct = 0;
  const misses: Array<{
...
describe("Local scanning accuracy (stop condition: >=99%)", () => {
  it("meets accuracy threshold across SOTA datasets", async () => {
    ...
    if (openphish.length) {
      await localThreatDb.ingestThreatUrls(
        "openphish",
        openphish.map((r) => r.url),
      );
    }
    if (urlhaus.length) {
      await localThreatDb.ingestThreatUrls(
    ...
    try {
      hardModeSuspicious = await buildHardModeSuspiciousFromReport(
        REPORT_PATH_DEFAULT,
        200,
      );
    } catch {
    ...
    const datasets: Array<{ name: string; rows: LabeledUrl[] }> = [
      ...
      ...(hardModeSuspicious.length
        ? [{ name: "hard-mode-report", rows: hardModeSuspicious }]
        : []),
    ].filter((d) => d.rows.length > 0);

    expect(
      datasets.length,
      "No datasets were fetched; cannot evaluate accuracy.",
    ).toBeGreaterThan(0);

    const results = await Promise.all(
      datasets.map(async (ds) => ({
        name: ds.name,
        ...(await measureAccuracy(localThreatDb, ds.rows)),
      })),
    );

    const failures = results.filter((r) => r.accuracy < 0.99);
    const summary = results
      .map((r) => `${r.name}=${(r.accuracy * 100).toFixed(2)}%`)
      .join(", ");
    const details = failures
      .map(
        (f) =>
          `${f.name} misses: ${f.misses.map((m) => `${m.expected}->${m.predicted} ${m.url}`).join(" | ")}`,
      )
      .join("\n");

    expect(
      failures,
      `Accuracy stop condition failed: ${summary}\n${details}`,
    ).toEqual([]);
  });
});

async function safeFetchUrls(
  fn: () => Promise<LabeledUrl[]>,
): Promise<LabeledUrl[]> {
  try {
    return await fn();
  } catch {
