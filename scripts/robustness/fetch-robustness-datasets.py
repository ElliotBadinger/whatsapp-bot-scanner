...
def parse_csv_file(
    ...
    with open(file_path, "r", encoding="utf-8", errors="ignore") as handle:
        ...
        for row in reader:
            ...
            write_entry(output_file, url, label, source_id, fetched_at, counts)
...
def parse_json_file(
    ...
    if size > 50 * 1024 * 1024:
        with open(file_path, "r", encoding="utf-8", errors="ignore") as handle:
            parse_json_lines(handle, output_file, counts, source_id, fetched_at, default_label)
        return
    with open(file_path, "r", encoding="utf-8", errors="ignore") as handle:
        try:
...
def parse_text_file(
    ...
    with open(file_path, "r", encoding="utf-8", errors="ignore") as handle:
        for line in handle:
            ...
            if not url:
                continue
            write_entry(output_file, url, default_label, source_id, fetched_at, counts)
...
def parse_dataset_tree(
    ...
    for file_path in iter_candidate_files(root_dir):
        ...
        if suffix in (".csv", ".tsv"):
            parse_csv_file(file_path, output_file, counts, source_id, fetched_at, label_hint)
        elif suffix in (".jsonl", ".ndjson"):
            with open(file_path, "r", encoding="utf-8", errors="ignore") as handle:
                parse_json_lines(handle, output_file, counts, source_id, fetched_at, label_hint)
        elif suffix == ".json":
            parse_json_file(file_path, output_file, counts, source_id, fetched_at, label_hint)
        else:
            parse_text_file(file_path, output_file, counts, source_id, fetched_at, label_hint)


def fetch_huggingface(source: Dict, output_path: Path, fetched_at: str) -> Dict:
    try:
    ...
    with open(output_path, "w", encoding="utf-8") as output_file:
        for row in dataset:
            ...
            write_entry(output_file, url, label, source["id"], fetched_at, counts)
    return counts


def fetch_github_zip(source: Dict, output_path: Path, cache_dir: Path, force: bool, fetched_at: str) -> Dict:
    repo = source["repo"]
    ...
    try:
        ...
        counts: Dict[str, int] = {}
        with open(output_path, "w", encoding="utf-8") as output_file:
            parse_dataset_tree(temp_dir, output_file, counts, source["id"], fetched_at)
        return counts
    finally:
...
def fetch_archive_source(source: Dict, output_path: Path, cache_dir: Path, force: bool, fetched_at: str) -> Dict:
...
def fetch_dynapd_kits(source: Dict, output_path: Path, cache_dir: Path, force: bool, fetched_at: str) -> Dict:
    archive_path = resolve_archive_path(source, cache_dir, force)
    if not archive_path or not archive_path.exists():
        raise RuntimeError("Archive not available")
    temp_dir = Path(tempfile.mkdtemp(prefix=f"{source['id']}-"))
...
def fetch_threatfox(source: Dict, output_path: Path, cache_dir: Path, force: bool, fetched_at: str) -> Dict:
    archive_path = cache_dir / f"{source['id']}.zip"
    ...
    try:
        ...
        with open(output_path, "w", encoding="utf-8") as output_file:
            for json_file in json_files:
                parse_json_file(json_file, output_file, counts, source["id"], fetched_at, "malicious")
        return counts
    finally:
        shutil.rmtree(temp_dir, ignore_errors=True)


def fetch_sslbl(source: Dict, output_path: Path, cache_dir: Path, force: bool, fetched_at: str) -> Dict:
    ...
    with open(output_path, "w", encoding="utf-8") as output_file:
        with open(archive_path, "r", encoding="utf-8", errors="ignore") as handle:
            for line in handle:
                ...
                if not url:
                    continue
                write_entry(output_file, url, "malicious", source["id"], fetched_at, counts)
    return counts


def fetch_urlscan_export(source: Dict, output_path: Path, fetched_at: str) -> Dict:
    ...
    with open(output_path, "w", encoding="utf-8") as output_file:
        if export_path.suffix.lower() in (".jsonl", ".ndjson"):
            with open(export_path, "r", encoding="utf-8", errors="ignore") as handle:
                parse_json_lines(handle, output_file, counts, source["id"], fetched_at, "suspicious")
        else:
            parse_json_file(export_path, output_file, counts, source["id"], fetched_at, "suspicious")
    return counts
...
def fetch_report_patterns(source: Dict, output_path: Path, fetched_at: str, reports_dir: Path) -> Dict:
    counts: Dict[str, int] = {}
    patterns = extract_patterns_from_reports(reports_dir)
    with open(output_path, "w", encoding="utf-8") as output_file:
        for url in patterns:
            write_entry(output_file, url, "tricky", source["id"], fetched_at, counts)
    return counts
    ...
    for source in DATASET_SOURCES:
        ...
        try:
            if source["type"] == "huggingface":
                counts = fetch_huggingface(source, output_path, fetched_at)
            elif source["type"] == "github_zip":
                counts = fetch_github_zip(source, output_path, cache_dir, args.force, fetched_at)
            elif source["type"] == "archive":
                counts = fetch_archive_source(source, output_path, cache_dir, args.force, fetched_at)
            elif source["type"] == "archive_kits":
                counts = fetch_dynapd_kits(source, output_path, cache_dir, args.force, fetched_at)
            elif source["type"] == "threatfox":
                counts = fetch_threatfox(source, output_path, cache_dir, args.force, fetched_at)
            elif source["type"] == "sslbl":
                counts = fetch_sslbl(source, output_path, cache_dir, args.force, fetched_at)
            elif source["type"] == "urlscan_export":
                counts = fetch_urlscan_export(source, output_path, fetched_at)
            elif source["type"] == "report_patterns":
                counts = fetch_report_patterns(source, output_path, fetched_at, reports_dir)
            else:
