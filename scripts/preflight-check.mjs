...
function checkWhatsAppLibrary() {
  ...
  if (library === "baileys") {
    ...
    try {
      ...
      log(
        WARN,
        `  No Chromium/Chrome browser found (Puppeteer will download one)`,
      );
    }
  } else {
...
function checkDockerNetworking() {
  ...
  try {
    const nftOutput = execSync(
      "sudo nft list table ip waydroid_filter 2>/dev/null || true",
      {
        stdio: "pipe",
        encoding: "utf-8",
      },
    ...
    if (nftOutput.includes("policy drop")) {
      log(
        WARN,
        `Waydroid nftables rules detected - may block Docker networking`,
      );
...
async function main() {
  ...
  console.log(
    `${colors.blue}║${colors.reset}    WBScanner Pre-flight Check                  ${colors.blue}║${colors.reset}`,
  );
  console.log(
    `${colors.blue}╚════════════════════════════════════════════════╝${colors.reset}`,
  );
