  ...
  return {
    ...
    box(content, options = {}) {
      ...
      if (title) {
        ...
        top =
          borderColor(chars.tl + chars.h.repeat(titlePos)) +
          ` ${theme.textBold(title)} ` +
          borderColor(
            chars.h.repeat(width - titlePos - title.length - 2) + chars.tr,
          );
      }
      console.log(top);
      ...
      for (const line of lines) {
        const padding = " ".repeat(
          Math.max(0, width - stripAnsi(line).length - 2),
        );
        console.log(
          borderColor(chars.v) +
            " " +
    ...
    progressTable(items) {
      ...
      for (const item of items) {
        ...
        const detail = item.detail ? theme.muted(` (${item.detail})`) : "";
