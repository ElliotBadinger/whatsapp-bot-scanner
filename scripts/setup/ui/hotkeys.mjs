...
const GLOSSARY = [
  {
    term: "Guided Mode",
    definition: "Step-by-step setup with detailed explanations",
  },
  { term: "Expert Mode", definition: "Condensed output for experienced users" },
  {
    term: "Checkpoint",
    definition: "Saved progress point for resume capability",
  },
  { term: "Transcript", definition: "Log file recording all setup decisions" },
  {
    term: "Quick Action",
    description: "Shortcut commands like --quick=preflight",
  },
];
...
export function describeHotkeys() {
  return HOTKEYS.map((h) => `${C.accent(h.key)}:${h.description}`).join(
    C.muted(" │ "),
  );
}
...
export function getHotkeyHelp() {
  const lines = ["", C.textBold("  Keyboard Shortcuts:"), ""];

  for (const h of HOTKEYS) {
    lines.push(
      `    ${C.accent(h.key)}  ${C.muted("→")}  ${C.text(h.description)}`,
    );
  }
    ...
    switch (char) {
      ...
      case "R": {
        ...
        if (recoveryManager?.displayQuickActions) {
          ...
          console.log(
            `    ${C.code("./setup.sh --quick=preflight")}    ${C.muted("Run preflight only")}`,
          );
          console.log(
            `    ${C.code("./setup.sh --quick=resume-docker")} ${C.muted("Resume from Docker")}`,
          );
          console.log(
            `    ${C.code("./setup.sh --quick=purge-caches")}  ${C.muted("Clear caches")}\n`,
          );
        }
        break;
      }
