...
const GLOSSARY_ENTRIES = [
  ...
  [
    "Checkpoint",
    "Saved stage used for resume commands (preflight, environment, containers).",
  ],
  ["Transcript", "Markdown + JSON artifact capturing decisions and outcomes."],
  [
    "Quick action",
    "One-command helpers such as --quick=preflight or --quick=purge-caches.",
  ],
];

export function describeHotkeys() {
  return HOTKEY_DESCRIPTIONS.map(
    (item) => `${item.key.toUpperCase()}: ${item.description}`,
  ).join(" | ");
}
