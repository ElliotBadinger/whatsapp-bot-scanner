...
async function runConfigValidation({ context, output }) {
  ...
  await runWithSpinner(context, 'node scripts/validate-config.js', () =>
    execa('node', ['scripts/validate-config.js'], { cwd: ROOT_DIR, stdio: 'inherit' })
  );
...
export default {
  ...
  copy: {
    guided: {
      description: 'Ensure configuration files align with expected schema before launching services.'
    },
    expert: {
      description: 'Runs scripts/validate-config.js to confirm workspace integrity.'
    }
  },
