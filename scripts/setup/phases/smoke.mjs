...
async function offerPairingWatcher({ context, runtime, output }) {
  ...
  if (
    (runtime.envFile.get("WA_AUTH_STRATEGY") || "remote").toLowerCase() !==
    "remote"
  )
    return;
  if (
  ...
  await execa("npm", ["run", "watch:pairing-code"], {
    cwd: ROOT_DIR,
    stdio: "inherit",
  });
}
...
export default {
  ...
  copy: {
    guided: {
      description:
        "Run the smoke test and highlight observability endpoints for post-setup review.",
    },
    expert: {
      description:
        "Pings status endpoints and surfaces any pending integrations or disabled features.",
    },
  },
