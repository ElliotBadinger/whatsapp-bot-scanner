...
export default {
  ...
  copy: {
    guided: {
      description:
        "Sanity check third-party integrations to surface missing or invalid credentials early.",
    },
    expert: {
      description:
        "Performs lightweight API probes and records missing keys for follow-up.",
    },
  },
  async run({ context, runtime, output }) {
    ...
    if (!context.flags.noninteractive) {
      const optional = API_INTEGRATIONS.filter(
        (item) => item.importance === "optional",
      );
      if (optional.length) {
        output.note(
          "Optional integrations can be configured later via ./setup.sh.",
        );
      }
    }
  },
