...
function urlscanStep({ context, runtime, output }) {
  ...
  return (async () => {
    ...
    if (!choice) {
      output.info('urlscan.io kept disabled. Enable later via ./setup.sh or .env.');
      return;
    }
    const key = await prompt.input({
      ...
      validate: value => (value.trim().length < 32 ? 'Provide a valid token from https://urlscan.io/user/api.' : true)
    });
    ...
    output.info('urlscan.io integration configured. Why this matters: adds screenshot + DOM artifacts.', {
...
function dockerProfileStep({ context, runtime, output }) {
  ...
  return (async () => {
    ...
    const profiles = await prompt.input({
      ...
      initial: current,
      validate: value => (value.trim().length === 0 ? 'Enter at least one profile or cancel.' : true)
    });
    env.set('DOCKER_COMPOSE_PROFILES', profiles.split(',').map(v => v.trim()).filter(Boolean).join(','));
    output.info('Docker profiles updated. Why this matters: adjust resource footprint per environment.', {
      docs: 'docs/ARCHITECTURE.md'
    });
    context.recordDecision('docker.profiles', env.get('DOCKER_COMPOSE_PROFILES'));
  })();
}
...
export function registerBuiltinPlugins() {
  ...
  registerPlugin({
    ...
    description: 'Advanced operators can select non-default docker compose profiles.',
    optional: true,
    isEnabled(context) {
      return !context.flags.noninteractive;
    },
