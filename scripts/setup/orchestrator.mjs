...
async function offerPairingWatcher(context, output) {
  ...
  if (
    ...
    output.info(
      "Need audio cues later? Run bun run watch:pairing-code for audible alerts.",
    );
    return;
  }
  ...
  if (startWatcher) {
    ...
    try {
      ...
      await execa(pkgMgr, ["run", "--silent", "watch:pairing-code"], {
        cwd: ROOT_DIR,
        stdio: "inherit",
      });
    } catch {
      output.warn(
        "Pairing watcher exited unexpectedly. You can run bun run watch:pairing-code again later.",
      );
    }
  }
}
