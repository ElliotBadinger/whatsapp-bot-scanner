...
`;
  ...
  async step3ApiKeys() {
    ...
    if (!vtKey) {
      if (this.nonInteractive) {
        throw new Error(
          "VT_API_KEY required. Set it in .env before running with --noninteractive",
        );
      }
      ...
      console.log(
        `  ${ICON.success}  ${C.success("VirusTotal API key saved")}`,
      );
    } else {
    ...
    if (!phone) {
      if (this.nonInteractive) {
        throw new Error(
          "WA_REMOTE_AUTH_PHONE_NUMBERS required in non-interactive mode",
        );
      }
      ...
      console.log(
        `  ${ICON.success}  ${C.text(`Phone number configured: ${masked}`)}`,
      );
    }
