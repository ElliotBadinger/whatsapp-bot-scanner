...
export class DependencyError extends SetupError {
  constructor(dependency, message, cause = null, context = null) {
    super(`Dependency error (${dependency}): ${message}`, cause, 'critical', context);
    this.name = 'DependencyError';
    this.dependency = dependency;
    this.addRecoverySuggestion(`Please install ${dependency} and try again`);
    this.addRecoverySuggestion(`Run: sudo apt-get install ${dependency} (or equivalent for your OS)`);
  }
}
...
export class ConfigurationError extends SetupError {
  constructor(message, cause = null, context = null) {
    ...
    this.addRecoverySuggestion('Check your configuration files for syntax errors');
...
export class DockerError extends SetupError {
  constructor(message, cause = null, context = null) {
    ...
    this.addRecoverySuggestion('Verify Docker installation with: docker --version');
  }
}
...
export class DockerComposeError extends DockerError {
  constructor(message, cause = null, context = null) {
    ...
    this.addRecoverySuggestion('Please ensure Docker Compose v2 is installed and running');
    this.addRecoverySuggestion('Run: docker compose version');
    this.addRecoverySuggestion('If using legacy docker-compose, upgrade to Docker Compose v2');
  }
}

export class DockerContainerError extends DockerError {
  constructor(containerName, message, cause = null, context = null) {
    super(`Container error (${containerName}): ${message}`, cause, 'high', context);
    this.name = 'DockerContainerError';
    this.containerName = containerName;
    this.addRecoverySuggestion(`Container ${containerName} may need to be restarted`);
    this.addRecoverySuggestion(`Run: docker compose restart ${containerName}`);
    this.addRecoverySuggestion(`Check container logs: docker compose logs ${containerName}`);
  }
}
...
export class DockerLogStreamError extends DockerError {
  constructor(message, cause = null, context = null) {
    ...
    this.addRecoverySuggestion('Log streaming failed. Check if containers are running');
...
export class DockerHealthCheckError extends DockerError {
  constructor(serviceName, message, cause = null, context = null) {
    super(`Health check error (${serviceName}): ${message}`, cause, 'high', context);
...
export class PairingError extends SetupError {
  constructor(message, cause = null, context = null) {
    ...
    this.addRecoverySuggestion('Check your WhatsApp connection and credentials');
    this.addRecoverySuggestion('Verify your phone number format (include country code)');
...
export class ValidationError extends SetupError {
  constructor(field, message, cause = null, context = null) {
    ...
    this.field = field;
    this.addRecoverySuggestion(`Please check the ${field} field and correct the value`);
    this.addRecoverySuggestion('Verify the format and requirements for this field');
  }
}
...
export class NetworkError extends SetupError {
  constructor(message, cause = null, context = null) {
    ...
    this.addRecoverySuggestion('Verify network connectivity to required services');
    this.addRecoverySuggestion('Check firewall settings and proxy configurations');
  }
}

export class TimeoutError extends SetupError {
  constructor(operation, timeout, cause = null, context = null) {
    super(`Timeout error: ${operation} timed out after ${timeout}ms`, cause, 'medium', context);
    this.name = 'TimeoutError';
    this.operation = operation;
    this.timeout = timeout;
    this.addRecoverySuggestion(`The operation ${operation} took too long to complete`);
    this.addRecoverySuggestion('Check system resources and network connectivity');
    this.addRecoverySuggestion('Try increasing the timeout or retrying the operation');
  }
}

export class RetryError extends SetupError {
  constructor(operation, maxRetries, cause = null, context = null) {
    super(`Retry error: ${operation} failed after ${maxRetries} attempts`, cause, 'high', context);
    this.name = 'RetryError';
    this.operation = operation;
    this.maxRetries = maxRetries;
    this.addRecoverySuggestion(`The operation ${operation} failed after multiple attempts`);
    this.addRecoverySuggestion('Check the underlying issue causing the failure');
    this.addRecoverySuggestion('Consider manual intervention or different approach');
  }
}
...
export class GlobalErrorHandler {
  ...
  formatError(error, context) {
    ...
    if (context) {
      formatted.recoverySuggestions.push(`Check the ${context} component for issues`);
    }
  ...
  logError(formattedError) {
    ...
    // Log to console
    console.error(`[${formattedError.timestamp}] [${formattedError.severity.toUpperCase()}] ${formattedError.type}: ${formattedError.message}`);
  ...
  displayUserError(formattedError) {
    ...
    if (formattedError.recoverySuggestions && formattedError.recoverySuggestions.length > 0) {
    ...
    if (formattedError.severity === ERROR_SEVERITY.CRITICAL && formattedError.cause) {
      this.context?.ui?.info(`Cause: ${formattedError.cause.message || formattedError.cause}`);
    }
  }
  ...
  attemptRecovery(formattedError) {
    ...
    if (formattedError.context === 'docker' && this.context.dockerOrchestrator) {
      this.attemptDockerRecovery(formattedError);
    } else if (formattedError.context === 'pairing' && this.context.pairingManager) {
      this.attemptPairingRecovery(formattedError);
    } else if (formattedError.context === 'configuration' && this.context.configManager) {
  ...
  attemptDockerRecovery(error) {
    try {
      ...
      if (error.type === 'DockerContainerError' && error.containerName) {
        this.context.ui.info(`Attempting to restart container: ${error.containerName}`);
  ...
  attemptPairingRecovery(error) {
    try {
      ...
      if (error.type === 'PairingError') {
        ...
        if (this.context.pairingManager && typeof this.context.pairingManager.refreshPairingCode === 'function') {
  ...
  async retryOperation(operation, maxRetries = 3, initialDelay = 1000, context = null) {
    let attempt = 0;
    let lastError = null;
    ...
    while (attempt < maxRetries) {
      ...
      try {
        ...
        if (this.context && this.context.ui) {
          this.context.ui.warn(`Attempt ${attempt} failed. Retrying in ${delay/1000} seconds...`);
        }
  ...
  async withFallback(primaryOperation, fallbackOperation, context = null) {
    try {
      ...
      if (this.context && this.context.ui) {
        this.context.ui.warn(`Primary operation failed, attempting fallback: ${error.message}`);
      }
      ...
      try {
        ...
        if (this.context && this.context.ui) {
          this.context.ui.error(`Both primary and fallback operations failed: ${fallbackError.message}`);
        }
        throw fallbackError;
      }
    }
...
export class UserRecoverySystem {
  ...
  async showRecoveryMenu(error) {
    ...
    try {
      ...
      this.context.ui.error(`❌ Recovery action failed: ${recoveryError.message}`);
  ...
  addCommonRecoveryOptions(error) {
    ...
    if (error instanceof DependencyError) {
      ...
      this.addRecoveryOption({
        ...
        action: async () => {
          this.context.ui.info(`Manual installation instructions for ${error.dependency}:`);
      ...
      this.addRecoveryOption({
        ...
        action: async () => {
          if (this.context && this.context.dockerOrchestrator) {
            const status = await this.context.dockerOrchestrator.getServiceStatus();
    ...
    if (error instanceof PairingError) {
      ...
      this.addRecoveryOption({
        ...
        action: async () => {
          ...
          this.context.ui.info('1. Verify your phone number format (include country code)');
          this.context.ui.info('2. Check your internet connection');
          this.context.ui.info('3. Ensure WhatsApp is not running on another device');
    ...
    this.addRecoveryOption({
      ...
      action: async () => {
        ...
        if (this.context && this.context.envDetector) {
          ...
          this.context.ui.info(`Platform: ${envInfo.platform.platform} (${envInfo.platform.arch})`);
          this.context.ui.info(`Node.js: ${this.context.dependencyManager.getNodeVersion()}`);
  ...
  async executeWithFallbacks(primaryAction, fallbackActions = [], context = 'operation') {
    let lastError = null;
    ...
    for (let i = 0; i < fallbackActions.length; i++) {
      try {
        this.context.ui.info(`🔄 Attempting fallback ${i + 1}/${fallbackActions.length}...`);
        return await fallbackActions[i].action();
      } catch (fallbackError) {
        lastError = fallbackError;
        this.context.ui.warn(`Fallback ${i + 1} failed: ${fallbackError.message}`);
    ...
    // All attempts failed
    const finalError = new RetryError(context, fallbackActions.length + 1, lastError);
    finalError.addRecoverySuggestion('All recovery attempts failed');
    finalError.addRecoverySuggestion('Check system logs for detailed information');
    finalError.addRecoverySuggestion('Consider manual intervention or support request');
