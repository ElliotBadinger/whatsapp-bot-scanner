...
export class SetupWizard {
  ...
  showWelcomeMessage() {
    ...
    this.ui.info(
      "This will guide you through 5 simple steps to get everything running.\n",
    );
  }
  ...
  async step1PrerequisitesCheck() {
    ...
    await this.logger.info(
      `Starting Step ${this.currentStep}: Prerequisites Check`,
    );
    ...
    try {
      ...
      await this.logger.warn("Environment detection failed", {
        error: error.message,
      });
    }
    ...
    try {
      ...
      if (!this.dependencyManager.isVersionSufficient(nodeVersion)) {
        this.progress.fail(
          "node-check",
          `Node.js version ${nodeVersion} is insufficient`,
        );
    ...
    try {
      ...
      await this.logger.error("Dependency verification failed", {
        error: error.message,
      });
    ...
    await this.logger.logStepCompletion(
      this.currentStep,
      "Prerequisites Check",
      this.setupData.prerequisites,
    );
  ...
  async step2ApiKeysCollection() {
    this.currentStep = 2;
    await this.logger.info(
      `Starting Step ${this.currentStep}: API Keys Collection`,
    );
    ...
    try {
      ...
      await this.logger.error("Failed to load configuration", {
        error: error.message,
      });
    ...
    try {
      ...
      await this.logger.error("API key collection failed", {
        error: error.message,
      });
      throw new ConfigurationError(
        `Failed to collect API keys: ${error.message}`,
        error,
      );
    }
  }
  ...
  async step3WhatsAppPairing() {
    this.currentStep = 3;
    await this.logger.info(
      `Starting Step ${this.currentStep}: WhatsApp Pairing`,
    );
    ...
    const needsPairing =
      authStrategy === undefined || authStrategy === "remote";
    ...
    const pairingChoice = await this.ui.select({
      ...
      choices: [
        {
          name: "🤖 Automatic pairing (monitor for pairing codes)",
          value: "auto",
        },
    ...
    await this.logger.info("WhatsApp pairing method selected", {
      method: pairingChoice,
    });
    ...
    try {
      ...
      await this.logger.error("WhatsApp pairing failed", {
        error: error.message,
      });
  ...
  async handleAutomaticPairing() {
    ...
    this.ui.info(
      "This will monitor for pairing codes from the WhatsApp client.",
    );
    ...
    try {
      ...
      const simulatedCode =
        this.pairingManager.generateSimulatedPairingCode?.() ?? "123456";
      ...
      if (typeof this.pairingManager.monitorForPairingSuccess === "function") {
        ...
        const monitoringStarted =
          await this.pairingManager.monitorForPairingSuccess(
        ...
        this.ui.info(
          "🎯 Auto-detection enabled - will progress automatically when pairing succeeds",
        );
      }

      this.progress.succeed("pairing-monitor", "Pairing monitoring active ✓");
      await this.logger.info(
        "Pairing monitoring started successfully with auto-detection",
      );
    } catch (error) {
      this.progress.fail("pairing-monitor", "Pairing monitoring failed");
      await this.logger.error("Pairing monitoring failed", {
        error: error.message,
      });
      throw new PairingError(
        `Failed to start pairing monitoring: ${error.message}`,
        error,
      );
    }
  }
  ...
  async handlePairingSuccess(result) {
    try {
      ...
      this.ui.success(
        "✅ WhatsApp pairing setup complete - progressing to next step",
      );
      await this.logger.info(
        "WhatsApp pairing completed successfully with auto-detection",
      );
    } catch (error) {
      this.ui.error(`Error handling pairing success: ${error.message}`);
      await this.logger.error("Error handling pairing success", {
        error: error.message,
      });
    }
  }
  ...
  async handlePairingError(error) {
    try {
      ...
      await this.logger.error("WhatsApp pairing error detected", {
        error: error.message,
      });
    } catch (error) {
      this.ui.error(`Error handling pairing error: ${error.message}`);
      await this.logger.error("Error handling pairing error", {
        error: error.message,
      });
    }
  }
  ...
  async handleManualPairing() {
    ...
    try {
      ...
      await this.logger.error("Manual pairing failed", {
        error: error.message,
      });
  ...
  async step4StartingServices() {
    this.currentStep = 4;
    await this.logger.info(
      `Starting Step ${this.currentStep}: Starting Services`,
    );
    ...
    try {
      ...
      for (const service of servicesToMonitor) {
        try {
          const { stop } =
            await this.dockerOrchestrator.streamLogsWithFormatting(service, {
              tail: "50",
              timestamps: true,
              follow: true,
            });
          ...
          this.ui.warn(
            `⚠ Could not start log monitoring for ${service}: ${error.message}`,
          );
        }
      }
      ...
      this.progress.succeed(
        "service-status",
        `${runningServices.length} services healthy ✓`,
      );
      ...
      this.ui.success(
        "✅ All services started successfully with real-time monitoring",
      );
      await this.logger.logStepCompletion(
        this.currentStep,
        "Starting Services",
      ...
      await this.logger.error("Failed to start services", {
        error: error.message,
      });
  ...
  async step5Verification() {
    ...
    try {
      ...
      if (this.setupData.whatsappPairing?.method === "auto") {
        ...
        await this.logger.info(
          "WhatsApp connection: manual pairing configured",
        );
      } else {
      ...
      if (allServicesHealthy) {
        ...
        this.ui.success(
          "✅ Setup verification complete - all systems operational",
        );
