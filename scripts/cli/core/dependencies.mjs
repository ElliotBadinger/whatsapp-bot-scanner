...
export class DependencyManager {
  ...
  async installNodeViaNodeSource() {
    try {
      ...
      await execa("apt-get", ["install", "-y", "curl", "ca-certificates"], {
        stdio: "inherit",
      });
      ...
      throw new Error(
        `Failed to install Node.js via NodeSource: ${error.message}`,
      );
    }
  }
  ...
  async ensureDocker() {
    try {
      ...
      throw new Error(
        "Docker or Docker Compose not found. Please install Docker and Docker Compose v2.",
      );
    }
  }
