...
async function probe() {
  ...
  try {
    ...
    ['analyzerConnection', 'repositoryTarget', 'analyzer', 'analysisRun', 'analysisRunSummary', 'analysisCheck'].forEach(typeName => {
    ...
    if (repo.targets && repo.targets.edges.length > 0) {
      repo.targets.edges.forEach(edge => {
        ...
        console.log(`      ${target.isActivated ? '✅' : '❌'} ${path} (${target.ecosystem})`);
      });
    } else {
    ...
    if (edges.length > 0) {
      console.log(`   Found ${edges.length} recent runs (checking for latest):`);
      // Sort by createdAt descending to get true latest
      const sortedRuns = edges.map(e => e.node).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

      sortedRuns.forEach((run, index) => {
        console.log(`   ${index + 1}. ${run.status} (${run.commitOid.substring(0, 7)}) - ${new Date(run.createdAt).toLocaleString()}`);
    ...
    if (schema.__type && schema.__type.fields) {
      ...
      const interestingFields = ['analyzers', 'config', 'sca', 'dependencies', 'targets'];
      const found = fields.filter(f => interestingFields.some(i => f.toLowerCase().includes(i)));
