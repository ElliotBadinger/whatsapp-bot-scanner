...
async function collectSitemapUrls(rootUrl, seen = new Set()) {
  ...
  if (!res.ok) {
    console.error(
      `Failed to fetch sitemap ${sanitizeForLogging(rootUrl)}: ${res.status}`,
    );
    return [];
  }
...
async function crawlByLinks(startUrl) {
  ...
  if (!normalizedStart) {
    throw new Error(
      `Unable to normalize start URL: ${sanitizeForLogging(startUrl)}`,
    );
  }
  ...
  while (queue.length) {
    ...
    try {
      ...
      console.error(
        `Error crawling ${sanitizeForLogging(current)}:`,
...
async function fetchPageContent(url) {
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to fetch ${sanitizeForLogging(url)}: ${res.status}`,
    );
  }
  return res.text();
}
...
async function main() {
  ...
  console.log(
    `Collecting sitemap entries from ${sanitizeForLogging(SITEMAP_URL)}`,
  );
  ...
  for (const [index, url] of urls.entries()) {
    try {
      console.log(
        `[${index + 1}/${urls.length}] Fetching ${sanitizeForLogging(url)}`,
      );
      ...
      console.error(
        `Error processing ${sanitizeForLogging(url)}:`,
        sanitizeForLogging(error.message),
      );
    }
  }
