...
const SKIP_EXTENSIONS = new Set(['.png', '.jpg', '.jpeg', '.svg', '.json', '.xml', '.pdf', '.ico', '.gif', '.zip', '.map', '.webp', '.mp4', '.mp3']);
...
TURNDOWN.addRule('preserveCodeLanguage', {
  ...
  replacement: (_content, node) => {
    const codeChild = Array.from(node.childNodes || []).find((child) => child.nodeName === 'CODE');
    if (!codeChild) {
...
async function main() {
  ...
  console.log(`Export completed. Index written to ${path.relative(process.cwd(), indexPath)}`);
}
