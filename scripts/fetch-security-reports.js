        ...
        const req = https.request(DEEPSOURCE_API_ENDPOINT, options, (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    if (response.errors) {
                        reject(new Error(`DeepSource GraphQL Error: ${JSON.stringify(response.errors, null, 2)}`));
                    } else {
                        resolve(response.data);
                    }
                } catch (error) {
                    reject(new Error(`Failed to parse DeepSource response: ${error.message}`));
                }
            });
        });
...
async function sonarqubeRestRequest(endpoint) {
    ...
    if (!SONARQUBE_PROJECT_KEY) {
        console.log('⚠️  SONARQUBE_PROJECT_KEY not set, skipping SonarQube reports');
        return null;
    }
            ...
            res.on('end', () => {
                try {
                    ...
                    if (res.statusCode >= 400) {
                        reject(new Error(`SonarQube API Error (${res.statusCode}): ${JSON.stringify(response)}`));
                    } else {
                        resolve(response);
                    }
                } catch (error) {
                    reject(new Error(`Failed to parse SonarQube response: ${error.message}`));
                }
            });
        });
...
async function fetchDeepSourceSecurityReports() {
    ...
    try {
        ...
        reports.analysisRuns = runsData.repository.analysisRuns.edges.map(edge => edge.node);
...
async function fetchSonarQubeSecurityReports() {
    ...
    try {
        ...
        console.log(`✅ Security Rating: ${reports.measures?.component?.measures?.find(m => m.metric === 'security_rating')?.value || 'N/A'}`);
...
function saveReports(deepsourceReports, sonarqubeReports) {
    ...
    const deepsourceFile = path.join(OUTPUT_DIR, `deepsource-report-${TIMESTAMP}.json`);
    ...
    const deepsourceSecurityFile = path.join(OUTPUT_DIR, `deepsource-security-${TIMESTAMP}.json`);
    ...
    fs.writeFileSync(deepsourceSecurityFile, JSON.stringify(deepsourceSecurityReport, null, 2));
    console.log(`✅ DeepSource security report saved: ${deepsourceSecurityFile}`);

    // Save SonarQube reports
    if (sonarqubeReports) {
        const sonarqubeFile = path.join(OUTPUT_DIR, `sonarqube-report-${TIMESTAMP}.json`);
        ...
        const sonarqubeSecurityFile = path.join(OUTPUT_DIR, `sonarqube-security-${TIMESTAMP}.json`);
        const sonarqubeSecurityReport = {
            ...
            securityRating: sonarqubeReports.measures?.component?.measures?.find(m => m.metric === 'security_rating')
        };
        fs.writeFileSync(sonarqubeSecurityFile, JSON.stringify(sonarqubeSecurityReport, null, 2));
        console.log(`✅ SonarQube security report saved: ${sonarqubeSecurityFile}`);
    }

    // Create a combined summary report
    const summaryFile = path.join(OUTPUT_DIR, `security-summary-${TIMESTAMP}.json`);
    const summary = {
        metadata: {
            timestamp: new Date().toISOString(),
        ...
        deepsource: {
            ...
            criticalIssues: deepsourceReports.issues?.filter(i => i.severity === 'CRITICAL')?.length || 0
        },
        ...
        files: {
            ...
            sonarqubeReport: sonarqubeReports ? path.basename(path.join(OUTPUT_DIR, `sonarqube-report-${TIMESTAMP}.json`)) : null,
            sonarqubeSecurityReport: sonarqubeReports ? path.basename(path.join(OUTPUT_DIR, `sonarqube-security-${TIMESTAMP}.json`)) : null
        }
    };
    ...
    try {
        ...
        if (sonarqubeReports) {
            ...
            fs.copyFileSync(path.join(OUTPUT_DIR, `sonarqube-report-${TIMESTAMP}.json`), latestSonarQube);
        }
    } catch (err) {
