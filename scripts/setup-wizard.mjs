...
function parseArgs() {
  ...
  for (const arg of args) {
    ...
    else if (arg === "--skip-api-validation")
      CLI_FLAGS.skipApiValidation = true;
...
async function planWizard() {
  ...
  const selections = await new MultiSelect({
    name: "actions",
    message:
      "Choose any prep steps you would like us to handle before provisioning",
...
async function preflightChecks() {
  if (CLI_FLAGS.skipPreflight) {
    logWarn(
      "Skipping preflight checks (--skip-preflight provided). Docker requirements were not verified.",
    );
    return;
  }
...
async function verifyApiKeys() {
  ...
  if (CLI_FLAGS.skipApiValidation) {
    logWarn(
      "Skipping outbound API validation (--skip-api-validation provided).",
    );
    await notePhishTank();
