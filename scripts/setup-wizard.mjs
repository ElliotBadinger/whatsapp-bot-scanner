...
const ENV_TEMPLATE_PATH = resolveWithinRoot(
  process.env.SETUP_ENV_TEMPLATE_PATH,
  ".env.example",
);
...
const REQUIRED_COMMANDS = [
  ...
  {
    name: "make",
    hint: "Install via your package manager (e.g. `brew install make` or `sudo apt install make`).",
  },
...
modeManager.on("error", (error) => {
  if (error && error.message) {
    ...
    setupLogger.recordDecision("mode-preference-error", {
...
async function multiSelectPrompt(options) {
  ...
  try {
    ...
    setupLogger.recordPrompt("multi-select", options, answer, {
      interactive: true,
    });
    return answer;
  } finally {
  ...
  return definitions.map((entry) => {
    ...
    if (typeof entry.task === "function") {
      instrumented.task = async (ctx, task) => {
        const tracker = setupLogger.startTask(
          entry.title || task?.title || "task",
          {
            category: "listr",
...
async function parseArgs() {
  ...
  for (const arg of args) {
    if (arg === "--clean") CLI_FLAGS.clean = true;
      ...
      CLI_FLAGS.fromTarball = arg.split("=")[1];
...
async function welcome() {
  ...
  const confirm = await confirmPrompt({
    message:
      mode === MODES.EXPERT
        ? "Ready to kick off the setup run?"
        : "Ready to begin the guided setup?",
    initial: true,
  });
  if (!confirm) {
...
async function planWizard() {
  ...
  console.log(
    `• Pull latest code/images: ${CLI_FLAGS.pull ? chalk.green("Yes") : chalk.gray("No")}`,
  );
  console.log(
    `• Stop existing containers: ${CLI_FLAGS.clean ? chalk.green("Yes") : chalk.gray("No")}`,
  );
  console.log(
    `• Full reset (remove volumes): ${CLI_FLAGS.reset ? chalk.yellow("Yes – destructive") : chalk.gray("No")}`,
  );
  console.log(
    `• Target branch: ${CLI_FLAGS.branch ? chalk.green(CLI_FLAGS.branch) : chalk.gray("Stay on current")}`,
  );
  console.log("");
...
async function preflightChecks() {
  ...
  for (const cmd of commandList) {
    try {
      ...
      throw new Error(
        `Missing required command: ${cmd.name}\nInstall hint: ${cmd.hint}`,
      );
    }
  }
  if (skipDocker) {
  ...
  try {
    await execa(
      ...
      [...dockerComposeCommand.slice(1), "version"],
      { stdio: "ignore" },
    );
  } catch {
    throw new Error(
...
async function checkoutBranch() {
  ...
  await runWithSpinner("git fetch", () =>
...
async function pullUpdates() {
  ...
  await runWithSpinner("docker compose pull", () =>
    execa(dockerComposeCommand[0], [...dockerComposeCommand.slice(1), "pull"], {
      cwd: ROOT_DIR,
    }),
  );
}
...
async function configureSecrets() {
  ...
  if (!getEnvVar("URLSCAN_CALLBACK_URL")) {
    setEnvVar(
      ...
      "http://reverse-proxy:8088/urlscan/callback",
    );
  }
}
  ...
  for (const integration of API_INTEGRATIONS) {
    ...
    if (CLI_FLAGS.noninteractive) {
      if (integration.flag) {
        ...
        recordDisabledFeature(
          `${integration.title} disabled (non-interactive run).`,
        );
      }
      recordMissingKey(
        `${integration.title} (${integration.importance}) missing; re-run setup interactively.`,
      );
      continue;
    }
    ...
    if (!answer.trim()) {
      if (integration.flag) {
        setEnvVar(integration.flag, "false");
        recordDisabledFeature(
          `${integration.title} disabled until API key is provided.`,
        );
      }
      recordMissingKey(
        `${integration.title} (${integration.importance}) missing; add later for better coverage.`,
      );
    ...
    logSuccess(
      `${integration.title} key stored (redacted: ${redact(answer.trim())}).`,
    );
    if (
      integration.flag &&
      getEnvVar(integration.flag).toLowerCase() === "false"
    ) {
...
async function configureRemoteAuth() {
  if (
    !getEnvVar("WA_AUTH_STRATEGY") ||
    getEnvVar("WA_AUTH_STRATEGY") === "local"
  ) {
  ...
  if (phone === "27632710634" && defaultDigits) {
    ...
    logInfo(
      `Replaced legacy RemoteAuth phone with default (${redact(phone)}).`,
    );
  }
  ...
  if (!phone) {
    if (CLI_FLAGS.noninteractive) {
      logWarn(
        "WA_REMOTE_AUTH_PHONE_NUMBER not set; QR pairing will be required.",
      );
    } else {
...
async function promptForPhone(defaultDigits) {
  printHeading("WhatsApp RemoteAuth Phone", {
    guided:
      "Share the phone number that should receive WhatsApp pairing codes (digits only).",
    expert: "RemoteAuth phone",
  });
  logInfo(
    "Provide the phone number WhatsApp should send pairing codes to (digits only).",
  );
...
async function configureAutoPair(phoneDigits) {
  ...
  printHeading("Linking your WhatsApp", {
    guided:
      "Decide whether we should request the WhatsApp phone-code automatically.",
    expert: "RemoteAuth automation",
  });
  logInfo(
    `We can request a one-time code automatically and display it for ${redact(phoneDigits)}.`,
  );
  logInfo(
    "If you prefer to scan a QR code yourself, choose “No” and we will show the QR in the logs.",
  );
  ...
  if (enable) {
    ...
    logWarn(
      "Auto pairing disabled; you will scan a QR code after the stack starts.",
    );
  }
}
...
async function checkPorts() {
  ...
  if (collisions.length > 0) {
    ...
    for (const collision of collisions) {
      logWarn(
        `• Port ${collision.port} (${collision.label}) is already in use.`,
      );
...
async function runConfigValidation() {
  ...
  if (!(await commandExists("node"))) {
    logWarn(
      "Node.js not detected; skipping scripts/validate-config.js validation.",
    );
    return;
  }
...
async function cleanUpStack() {
  if (CLI_FLAGS.reset) {
    printHeading("Reset Requested", {
      guided:
        "A full reset removes database data and the WhatsApp session volume.",
    ...
    await runWithSpinner("docker compose down -v --remove-orphans", () =>
      execa(
        dockerComposeCommand[0],
        [...dockerComposeCommand.slice(1), "down", "-v", "--remove-orphans"],
        { cwd: ROOT_DIR },
      ),
    );
    return;
  ...
  if (CLI_FLAGS.clean) {
    ...
    await runWithSpinner("docker compose down", () =>
      execa(
        dockerComposeCommand[0],
        [...dockerComposeCommand.slice(1), "down"],
        { cwd: ROOT_DIR },
      ),
    );
  }
...
async function buildAndLaunch() {
  ...
  printHeading("Building containers", {
    guided:
      "Building workspace images; this may take a few minutes on first run.",
  ...
  await runWithSpinner("Stopping existing stack", () =>
    execa(
      dockerComposeCommand[0],
      [...dockerComposeCommand.slice(1), "down", "--remove-orphans"],
      { cwd: ROOT_DIR },
    ),
  );
  await runWithSpinner("Pruning stopped containers", () =>
    execa(
      dockerComposeCommand[0],
      [...dockerComposeCommand.slice(1), "rm", "-f"],
      { cwd: ROOT_DIR },
    ),
  );
  ...
  try {
    ...
    await runWithSpinner("docker compose up -d", () =>
      execa(
        dockerComposeCommand[0],
        [...dockerComposeCommand.slice(1), "up", "-d"],
        { cwd: ROOT_DIR, stdio: "inherit" },
      ),
    );
  }
...
async function waitForFoundations() {
  ...
  printHeading("Waiting for core services", {
    guided:
      "Watching containers until Postgres, Redis, and orchestrators report healthy.",
...
async function waitForContainerHealth(service, label) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    const { stdout } = await execa(
      dockerComposeCommand[0],
      [...dockerComposeCommand.slice(1), "ps", "-q", service],
      { cwd: ROOT_DIR },
    );
...
async function tailWhatsappLogs() {
  ...
  const autoPair =
    (getEnvVar("WA_REMOTE_AUTH_AUTO_PAIR") || "").toLowerCase() === "true";
  ...
  if (strategy === "remote" && phone && autoPair) {
    ...
    logInfo(
      "A QR code will appear below. Open WhatsApp → Linked Devices → Link a Device and scan it.",
    );
  }
  logInfo(
    "This log stream exits automatically once the client prints 'WhatsApp client ready'.",
  );
...
async function smokeTest() {
  ...
  logInfo(
    "Next steps: drop a harmless link (e.g., https://example.com) in your pilot group to confirm verdict logging.",
  );
  logInfo(
    "Admin commands (`!scanner status`, `!scanner mute`, `!scanner unmute`, `!scanner rescan <url>`) are available in group chats.",
  );
}
...
async function offerPairingWatcher() {
  if (CLI_FLAGS.noninteractive || CLI_FLAGS.dryRun) return;
  if ((getEnvVar("WA_AUTH_STRATEGY") || "remote").toLowerCase() !== "remote")
    return;
  ...
  if (startWatcher) {
    ...
    logInfo(
      "No problem. Run `npm run watch:pairing-code` later if you change your mind.",
    );
  }
}
...
async function main() {
  ...
  try {
          ...
          task: () =>
            new Listr(
              instrumentTaskDefinitions([
                ...
                {
                  title: "Checkout branch (if requested)",
                  task: checkoutBranch,
                },
    ...
    if (transcriptPaths) {
      ...
      logInfo(
        `Setup transcript saved to ${relativeMd} (JSON: ${relativeJson}).`,
      );
    }
  }
