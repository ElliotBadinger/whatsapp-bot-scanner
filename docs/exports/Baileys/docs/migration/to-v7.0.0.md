# Source: https://baileys.wiki/docs/migration/to-v7.0.0

<!-- Scraped from https://baileys.wiki/docs/migration/to-v7.0.0 -->

[Skip to main content](https://baileys.wiki/docs/migration/to-v7.0.0/#__docusaurus_skipToContent_fallback)
...
## LIDs[​](https://baileys.wiki/docs/migration/to-v7.0.0/#lids "Direct link to LIDs")

This system requires the auth state to support the `lid-mapping`, `device-list`, and `tctoken` keys. Look at the SignalDataTypeMap to see what needs change in your application. Make sure you have updated your authentication state.
...
Also, in the [GroupMetadata](https://baileys.wiki/docs/api/interfaces/GroupMetadata) type, each ID type is now a LID and associated with it is a pn type (`owner` and `ownerPn`, `descOwner` and `descOwnerPn`, so on..)
In the [Contact](https://baileys.wiki/docs/api/interfaces/Contact) type, there are no longer any `jid`/`lid` fields. Instead, there is an `id` field (the preferred one by WhatsApp), and there is a `phoneNumber` and `lid` field. One or the other is present depending on the `id` field. If the `id` is an LID, then the `phoneNumber` is present, and vice versa.
NOTE: The changes applied in the [Contact](https://baileys.wiki/docs/api/interfaces/Contact) type affect the [`participants`](https://baileys.wiki/docs/api/interfaces/GroupMetadata#participants) property of groups as well.
...
## Acks[​](https://baileys.wiki/docs/migration/to-v7.0.0/#acks "Direct link to Acks")

We no longer send ACKs on successful message delivery. WhatsApp seems to be banning users for this.

## Meta Coexistence[​](https://baileys.wiki/docs/migration/to-v7.0.0/#meta-coexistence "Direct link to Meta Coexistence")
...
## ESM[​](https://baileys.wiki/docs/migration/to-v7.0.0/#esm "Direct link to ESM")
...
1. Convert your project to ESM (recommended): This involves changing your type in `package.json` to `module` and replacing `require()` calls with `import` calls. If you have any remaining CommonJS modules that you still need to use from ESM, you can use [`createRequire()`](https://nodejs.org/api/module.html#modulecreaterequirefilename).
2. Import Baileys from within CommonJS: This isn't recommended, but you can do this by using a `await import()` call. Example:
...
## Protobufs[​](https://baileys.wiki/docs/migration/to-v7.0.0/#protobufs "Direct link to Protobufs")
...
[](https://github.com/WhiskeySockets/baileys.wiki-site/tree/main/docs/migration/to-v7.0.0.md)
[Previous Introduction](https://baileys.wiki/docs/intro)[Next Socket](https://baileys.wiki/docs/category/socket)

- [LIDs](https://baileys.wiki/docs/migration/to-v7.0.0/#lids)
- [Acks](https://baileys.wiki/docs/migration/to-v7.0.0/#acks)
- [Meta Coexistence](https://baileys.wiki/docs/migration/to-v7.0.0/#meta-coexistence)
- [ESM](https://baileys.wiki/docs/migration/to-v7.0.0/#esm)
- [Protobufs](https://baileys.wiki/docs/migration/to-v7.0.0/#protobufs)
