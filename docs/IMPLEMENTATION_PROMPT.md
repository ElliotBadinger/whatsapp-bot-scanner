...
async function runPreflightChecks(): Promise<PreflightResult> {
  ...
  return {
    ...
    checks,
    readyForPairing:
      checks.redisConnectivity &&
      checks.whatsappWebReachable &&
      checks.phoneNumberConfigured,
  };
}
```
...
const libraryChoice = await prompt.select({
  ...
  choices: [
    {
      name: "baileys",
      message: "Baileys (Recommended) - Protocol-based, lightweight",
      value: "baileys",
    },
    {
      name: "wwebjs",
      message: "whatsapp-web.js - Browser-based, legacy support",
      value: "wwebjs",
    },
  ],
  initial: 0,
