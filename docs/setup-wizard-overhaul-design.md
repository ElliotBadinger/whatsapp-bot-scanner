...
- **Testing Hooks:** Introduce a dry-run flag, environment overrides (`SETUP_ENV_PATH`, `SETUP_SKIP_*`), and Vitest coverage (`tests/integration/setup-wizard.test.ts`) so CI can cover preflight + env mutations without Docker.
