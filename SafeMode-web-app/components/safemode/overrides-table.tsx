  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <span className="font-mono text-primary text-sm">ACTIVE OVERRIDES: {overrides?.length || 0}</span>
        <Button
      ...
      {/* Add form */}
      {isAdding && (
        <form onSubmit={handleAdd} className="border border-border bg-background/50 p-4 space-y-3">
      ...
      <div className="border border-border overflow-hidden">
        ...
        <div className="divide-y divide-border">
          {overrides?.map((override) => (
            ...
            >
              <span className="text-secondary truncate">{override.pattern}</span>
              <span className={cn(override.action === "allow" ? "text-success" : "text-danger")}>
                {override.action.toUpperCase()}
              </span>
              <span className="text-muted-foreground truncate">{override.reason}</span>
              <span className="text-muted-foreground/60">{new Date(override.createdAt).toLocaleDateString()}</span>
            </div>
          ))}
