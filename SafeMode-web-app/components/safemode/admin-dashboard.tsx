...
import { ApiError, logoutAdmin } from "@/lib/api";
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      if (err instanceof ApiError && err.status === 403) {
        setLogoutError("CSRF_FAILED: Refresh and try again");
      } else {
        setLogoutError("LOGOUT_FAILED: Unable to logout");
      }
    } finally {
      ...
      <main className="container mx-auto px-6 lg:px-10 py-8">
        ...
        <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
          ...
          <button
            ...
            className="font-mono text-xs text-danger/60 hover:text-danger transition-colors focus-ring px-3 py-1.5 border border-danger/30 hover:border-danger/60 disabled:opacity-70"
          >
            [ LOGOUT ]
          </button>
        </div>
        ...
        {logoutError && (
          <div className="mb-6 font-mono text-xs text-danger bg-danger/10 border border-danger/30 p-3">
            {logoutError}
          </div>
        )}
