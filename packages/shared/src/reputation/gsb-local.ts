...
export class GsbLocalDatabase {
  ...
  private async verifyWithRemoteApi(
    ...
    try {
      ...
      const confirmedMatches: GsbLocalThreatMatch[] = Array.isArray(
        json?.matches,
      )
        ? json.matches.map((m) => ({
      ...
      logger.warn(
        { err },
        "GSB remote verification failed, returning local matches",
      );
      return {
  ...
  async updateDatabase(): Promise<void> {
    try {
      ...
      for (const listUpdate of response.listUpdateResponses) {
        const { threatType, newClientState, additions, responseType } =
          listUpdate;
        ...
        if (additions) {
          for (const addition of additions) {
            if (addition.rawHashes) {
              ...
              const prefixSize =
                addition.rawHashes.prefixSize || HASH_PREFIX_LENGTH;
              ...
              for (
                ...
                const hashPrefix = buffer
                  .slice(offset, offset + prefixSize)
                  .toString("hex");
                const key = `${GSB_LOCAL_DB_PREFIX}${hashPrefix}`;
      ...
      logger.info(
        { totalAdditions, threatTypes: Object.keys(state) },
        "GSB local database updated successfully",
      );
    } catch (err) {
