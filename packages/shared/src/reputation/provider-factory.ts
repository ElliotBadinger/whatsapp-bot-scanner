...
export interface VirusTotalProvider {
  vtAnalyzeUrl: (
    url: string,
  ) => Promise<{ data?: unknown; latencyMs?: number; disabled?: boolean }>;
  vtVerdictStats: (analysis: {
    data?: unknown;
    disabled?: boolean;
...
export interface GsbProvider {
  gsbLookup: (
    urls: string[],
  ) => Promise<{ matches: unknown[]; latencyMs?: number }>;
}

export interface UrlhausProvider {
  urlhausLookup: (
    url: string,
  ) => Promise<{ listed: boolean; latencyMs?: number }>;
}

export interface PhishtankProvider {
  phishtankLookup: (
    url: string,
  ) => Promise<{ verified: boolean; latencyMs?: number }>;
}

export interface WhoisXmlProvider {
  whoisXmlLookup: (hostname: string) => Promise<{
...
export interface WhoDatProvider {
  whoDatLookup: (hostname: string) => Promise<{
    record?: { estimatedDomainAgeDays?: number; registrarName?: string };
  }>;
}
...
export async function getVirusTotalProvider(): Promise<VirusTotalProvider | null> {
  ...
  const cached = providerCache.get("virustotal") as
    | VirusTotalProvider
    | undefined;
...
export async function getPhishtankProvider(): Promise<PhishtankProvider | null> {
  ...
  const cached = providerCache.get("phishtank") as
    | PhishtankProvider
    | undefined;
  if (cached) return cached;
