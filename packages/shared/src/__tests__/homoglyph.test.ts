...
describe("homoglyph detection", () => {
  it("detects Cyrillic substitutions in PayPal impersonations", () => {
    ...
    const cyrillicA = result.confusableChars.find(
      (entry) => entry.original === "а" && entry.confusedWith === "a",
    );
    expect(cyrillicA).toBeDefined();
    expect(result.riskReasons).toEqual(
      expect.arrayContaining([
        expect.stringContaining("Mixed scripts detected"),
      ]),
    );
  });
  ...
  it("captures Greek homoglyphs in Google lookalikes", () => {
    ...
    expect(result.riskLevel).toBe("high");
    expect(
      result.confusableChars.filter((entry) => entry.original === "ο"),
    ).toHaveLength(2);
    expect(result.riskReasons).toEqual(
  ...
  it("treats punycode hostnames with confusables as high risk", () => {
    ...
    expect(result.riskReasons).toEqual(
      expect.arrayContaining([
        expect.stringContaining("punycode/IDN encoding"),
      ]),
    );
  });
  ...
  it("reports multiple confusable characters", () => {
    ...
    expect(result.riskLevel).toBe("high");
    const pairs = result.confusableChars.map(
      (entry) => `${entry.original}→${entry.confusedWith}`,
    );
