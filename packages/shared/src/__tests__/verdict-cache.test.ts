...
describe("VerdictCache", () => {
  ...
  test("sets and gets a verdict", () => {
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith(
      expect.objectContaining({ urlHash: hash }),
      "Cached verdict",
    );
    expect(mockLogger.debug).toHaveBeenCalledWith(
      expect.objectContaining({ urlHash: hash }),
  ...
  test("returns undefined for missing key", () => {
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith(
      expect.objectContaining({ urlHash: "missing" }),
      "Cache miss",
    );
  });

  test("respects ttl", async () => {
    const shortCache = new VerdictCache({
      ttlSeconds: 1,
  ...
  test("getTtl returns expiration time", () => {
    ...
    cache.set(
      "h1",
      { verdict: "benign", confidence: 1, timestamp: Date.now() },
      ttl,
    );
