...
describe('HTTP Fingerprinting', () => {
  describe('httpFingerprinting', () => {
    ...
    it('should handle connection timeout', async () => {
      const result = await httpFingerprinting('https://nonexistent-domain-12345.invalid/', {
        timeoutMs: 100,
        enableSSRFGuard: true,
      });
    ...
    it('should detect suspicious redirects', async () => {
      const result = await httpFingerprinting('https://httpbin.org/redirect-to?url=https://example.com', {
        timeoutMs: 3000,
        enableSSRFGuard: false,
      });
    ...
    it('should use random user agents', async () => {
      const result1 = await httpFingerprinting('https://httpbin.org/user-agent', {
        timeoutMs: 3000,
        enableSSRFGuard: false,
      });

      const result2 = await httpFingerprinting('https://httpbin.org/user-agent', {
        timeoutMs: 3000,
        enableSSRFGuard: false,
      });
