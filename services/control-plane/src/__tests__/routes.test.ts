...
describe("control-plane routes", () => {
  ...
  it("invalidates caches and enqueues rescan job", async () => {
    ...
    expect(redisClient.del).toHaveBeenCalledWith(
      `url:analysis:${body.urlHash}:vt`,
    );
    expect(redisClient.del).toHaveBeenCalledWith(
      `url:analysis:${body.urlHash}:gsb`,
    );
    expect(redisClient.del).toHaveBeenCalledWith(
      `url:analysis:${body.urlHash}:whois`,
    );
    expect(redisClient.del).toHaveBeenCalledWith(
      `url:analysis:${body.urlHash}:phishtank`,
    );
    expect(redisClient.del).toHaveBeenCalledWith(
      `url:analysis:${body.urlHash}:urlhaus`,
    );
    expect(redisClient.del).toHaveBeenCalledWith(
      `url:shortener:${body.urlHash}`,
    );
    expect(queue.add).toHaveBeenCalledWith(
      "rescan",
      expect.objectContaining({
        url: expect.stringMatching(/^https:\/\/example\.com\/test\/?$/),
  ...
  it("fails to start when control plane token missing", async () => {
    ...
    await expect(buildPromise as Promise<unknown>).rejects.toThrow(
      /CONTROL_PLANE_API_TOKEN must not be empty/,
    );
