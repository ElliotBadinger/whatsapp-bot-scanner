...
async function prepareSessionArtifacts(sessionName: string) {
  ...
  await fs.mkdir(path.join(dataPath, 'wwebjs_temp_session_default', 'Default'), { recursive: true });
  await fs.writeFile(path.resolve(`${sessionName}.zip`), Buffer.from('functional-session'));
  return { baseDir, dataPath };
}

async function cleanupSessionArtifacts(sessionName: string, baseDir: string) {
  await fs.rm(path.resolve(`${sessionName}.zip`), { force: true }).catch(() => {});
  await fs.rm(baseDir, { recursive: true, force: true }).catch(() => {});
}

function createGroupClient(group: any = { isGroup: true }) {
  return {
    getChatById: jest.fn(async () => group) as jest.MockedFunction<(chatId: string) => Promise<any>>,
  };
}

function createGetChatMock(chatId: string) {
  return jest.fn(async () => ({ id: { _serialized: chatId } })) as jest.MockedFunction<() => Promise<any>>;
}
...
function createMockMessageStore() {
  const store = new Map<string, { revocations: Array<{ scope: 'me' | 'everyone'; timestamp: number }> }>();
  return {
    async ensureRecord(details: { chatId: string; messageId: string; senderId?: string }) {
    ...
    async recordRevocation(chatId: string, messageId: string, scope: 'me' | 'everyone', timestamp: number) {
...
describe('wa-client functional flows', () => {
  it('force-new-session cleanup enables fresh chat operations', async () => {
    ...
    try {
      ...
      const reconstructedDir = path.join(dataPath, 'wwebjs_temp_session_default', 'Default');
      ...
      expect(record?.revocations.at(-1)).toMatchObject({ scope: 'me', timestamp: 456 });
    } finally {
  ...
  it('records message revoke without crashing', async () => {
    ...
    await expect(handleSelfMessageRevoke({
    ...
    expect(record?.revocations.at(-1)).toMatchObject({ scope: 'me', timestamp: 789 });
  ...
  it('survives force-new-session followed by message revoke sequence', async () => {
    ...
    try {
      ...
      expect(record?.revocations.at(-1)).toMatchObject({ scope: 'me', timestamp: 111 });
    } finally {
      await cleanupSessionArtifacts(sessionName, baseDir);
    }
  });
