...
describe('global token bucket limiter', () => {
  it('throttles the 1001st send within an hour', async () => {
    ...
    const limiter = createGlobalTokenBucket(fakeRedis, 1000, 'test_global_token_bucket');

    for (let i = 0; i < 1000; i += 1) {
      await expect(limiter.consume(GLOBAL_TOKEN_BUCKET_ID)).resolves.toMatchObject({ remainingPoints: expect.any(Number) });
    }
