...
export const config = {
  ...
  controlPlane: {
    get csrfToken(): string {
      if (process.env.CONTROL_PLANE_CSRF_TOKEN) return process.env.CONTROL_PLANE_CSRF_TOKEN.trim();
      const baseToken = process.env.CONTROL_PLANE_API_TOKEN || "test-token";
      return crypto.createHash('sha256').update(baseToken).update('csrf-salt').digest('hex');
    },
  },
  wa: {
    authStrategy: "remote",
...
export const hashChatId = (input: string) => `hash:${input}`;
export const hashMessageId = (input: string) => `hash:${input}`;
