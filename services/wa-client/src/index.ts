...
async function main() {
  ...
  app.post("/pair", async (_req, reply) => {
    if (!pairingOrchestrator) {
      return reply
        .code(400)
        .send({ error: "Pairing orchestrator not available" });
    }
  ...
  const performPairingCodeRequest = async (): Promise<string | null> => {
    ...
    const outcome = await pageHandle.evaluate(
      async (
        phoneNumber: string,
  ...
  pairingOrchestrator =
    shouldRequestPhonePairing && remotePhone
    ...
    try {
      ...
      if (notificationType === "promote" && recipients.length > 0) {
        ...
        await chat.sendMessage(lines.join(" "), {
          mentions: recipients as unknown as string[],
        });
...
export async function handleAdminCommand(
  ...
  if (cmd === "mute") {
    ...
    try {
      ...
      if (!resp.ok) {
        logger.warn(
          { status: resp.status, chatId: gc.id._serialized },
    ...
    try {
      ...
      await chat.sendMessage(
        `Unable to approve ${target}. Check logs for details.`,
      );
    }
    ...
    if (requested) {
      ...
      logger.info(
        { chatId: gc.id._serialized, senderId },
