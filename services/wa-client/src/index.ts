    ...
    if (sessionExists && config.wa.remoteAuth.forceNewSession) {
      ...
      try {
        ...
        await forceRemoteSessionReset({
          deleteRemoteSession: (name: string) =>
...
const MAX_PAIRING_CODE_RETRIES = FORCE_PHONE_PAIRING
  ? 10
  : CONFIGURED_MAX_PAIRING_RETRIES; // Cap forced retries to 10 to prevent infinite loops
...
async function deliverVerdictMessage(
  ...
  if (job.isCorrection) {
    try {
      ...
      const previousVerdictRecord =
        await messageStore.getVerdictRecord(context);
      if (previousVerdictRecord?.verdictMessageId) {
        try {
          const previousVerdictMessage = await client.getMessageById(
            previousVerdictRecord.verdictMessageId,
          );
            ...
            logger.info(
              {
                ...
                verdictMessageId: previousVerdictRecord.verdictMessageId,
              },
              "Deleted previous benign verdict message",
            );
            metrics.waMessageEdits.labels("correction_delete").inc();
          }
        } catch (err) {
          logger.warn(
            { err, verdictMessageId: previousVerdictRecord.verdictMessageId },
            "Failed to delete previous verdict message, will send correction as new message",
          );
        }
      }
    } catch (err) {
      logger.warn(
        { err, context },
        "Failed to retrieve previous verdict for correction editing",
      );
    }
  }
...
async function main() {
  ...
  app.post(
    "/pair",
    async (
      req: FastifyRequest<{ Body: { force?: boolean } }>,
      reply: FastifyReply,
    ) => {
  ...
  const shouldAutoStartPairing =
    shouldUsePhonePairingMode && config.wa.remoteAuth.autoPair;
  ...
  if (shouldUsePhonePairingMode && remotePhone) {
    ...
    logger.info(
      ...
      "Configured library for phone-number pairing mode",
    );
  }
  ...
  if (shouldUsePhonePairingMode) {
    if (config.wa.remoteAuth.autoPair) {
      ...
      logger.info(
        { phoneNumbers: remotePhoneNumbers.map(maskPhone) },
        "Phone-number pairing mode active. QR codes suppressed. Use /pair endpoint or setup wizard to request pairing code.",
      );
    }
  } else if (!remoteSessionActive && remotePhoneNumbers.length === 0) {
    logger.info(
      "No phone numbers configured and no session exists; QR code will be displayed for linking.",
    );
  }
  ...
  const waitForAuthStoreReady = async (maxWaitMs = 30000): Promise<boolean> => {
    ...
    const pupPage = (client as any).pupPage as
      ...
      try {
        ...
        if (isReady) {
          logger.debug(
            ...
            "AuthStore.PairingCodeLinkUtils is ready",
          );
          return true;
        }
      } catch (err) {
        ...
        logger.debug(
          ...
          "AuthStore check failed, retrying...",
        );
      }
    ...
    logger.warn(
      { maxWaitMs },
      "AuthStore.PairingCodeLinkUtils not ready after timeout",
    );
    return false;
  };
  ...
  const performPairingCodeRequestForPhone = async (
    ...
    if (!authStoreReady) {
      logger.warn(
        ...
        "AuthStore not ready, will retry pairing code request later",
      );
      return null;
    }
    ...
    try {
      ...
      logger.info(
        { phoneNumber: maskPhone(phone) },
        "Requesting pairing code via library method",
      );
      const code = await client.requestPairingCode(phone, true);
      if (code && typeof code === "string") {
        logger.info(
          { phoneNumber: maskPhone(phone), codeLength: code.length },
          "Pairing code received from library",
        );
        return code;
      }
      logger.warn(
        { phoneNumber: maskPhone(phone), code },
        "Invalid pairing code response from library",
      );
      return null;
    } catch (err) {
      const errMsg = err instanceof Error ? err.message : String(err);
      logger.error(
        { err: errMsg, phoneNumber: maskPhone(phone) },
        "Failed to request pairing code via library",
      );
      ...
      if (errMsg.includes("Evaluation failed")) {
        logger.warn(
          { phoneNumber: maskPhone(phone) },
          "WhatsApp page not ready for pairing, will retry later",
        );
        return null;
      }
      ...
      logger.info(
        { phoneNumber: maskPhone(phone) },
        "Falling back to Puppeteer pairing code request",
      );
      const pageHandle = getPageHandle(client);
      if (!pageHandle) {
        logger.warn(
          { phoneNumber: maskPhone(phone) },
          "Puppeteer page handle unavailable for pairing fallback",
        );
        return null;
      }
            ...
            const formattedError =
              err instanceof Error
                ...
                : errorInfo?.type === "rate_limit"
                  ? {
                      type: "rate_limit",
                      message: "WhatsApp API rate limit (429)",
                    }
                  : err;
      ...
      if (!pairingCodeDelivered && !remoteSessionActive) {
        ...
        if (FORCE_PHONE_PAIRING) {
          if (consecutiveAutoRefreshes >= MAX_CONSECUTIVE_AUTO_REFRESHES) {
            logger.warn(
              { phoneNumber: maskPhone(remotePhone) },
              'Pairing paused after multiple timeouts to prevent rate limits. Run "!scanner pair" to resume.',
            );
            if (config.wa.qrTerminal) {
              process.stdout.write(
  ...
  const executeReadyLogic = async (source: string) => {
    ...
    logger.info(
      { botWid, source },
      "Bot WID assigned, now listening for messages",
    );
    try {
  ...
  client.on("authenticated", () => {
    logger.info(
      "WhatsApp client authenticated - session received from WhatsApp",
    );
    ...
    setTimeout(() => {
      readyFallbackTimer = setInterval(async () => {
        ...
        try {
          ...
          if (client.info?.wid) {
            logger.info(
              { pollCount, botWid: client.info.wid._serialized },
              "Detected client.info.wid populated - triggering ready fallback",
            );
          ...
          const pupPage = (client as any).pupPage as
            | { evaluate: (fn: () => unknown) => Promise<unknown> }
            | undefined;
          if (pupPage && pollCount >= 10) {
            // Wait at least 20 seconds before trying Puppeteer fallback
            try {
              ...
              if (storeInfo.hasWid && storeInfo.wid) {
                logger.info(
                  { pollCount, storeInfo },
                  "Puppeteer fallback: Detected window.Store with user info - triggering ready",
                );
                ...
                logger.debug(
                  { pollCount, storeInfo },
                  "Puppeteer fallback: Store state check",
                );
              }
            } catch (evalErr) {
              ...
              if (pollCount % 15 === 0) {
                logger.debug(
                  {
                    err: evalErr instanceof Error ? evalErr.message : evalErr,
                    pollCount,
                  },
                  "Puppeteer fallback evaluation error",
                );
              }
            }
          }
          ...
          if (pollCount >= maxPolls) {
            logger.error(
              { pollCount, hasInfo: !!client.info, state },
              "Ready event fallback: Max polls reached without detecting ready state",
            );
            if (readyFallbackTimer) {
              clearInterval(readyFallbackTimer);
              readyFallbackTimer = null;
            ...
            logger.info(
              { pollCount, hasInfo: !!client.info, state },
              "Ready fallback: Still waiting for client.info to be populated",
            );
          }
        } catch (err) {
          if (pollCount % 15 === 0) {
            logger.debug(
              { err: err instanceof Error ? err.message : err, pollCount },
              "Ready fallback poll error (may be normal during init)",
            );
          }
        }
      }, 2000);
  ...
  if (
    pairingOrchestrator &&
    !pairingCodeDelivered &&
    (FORCE_PHONE_PAIRING || !useManualOnlyMode)
  ) {
...
> = {
  mute: async ({ chat, base, authHeaders, controlPlaneEnabled }) => {
    if (!controlPlaneEnabled || !authHeaders) {
      await chat.sendMessage("Control-plane disabled in MVP mode.");
      return;
    }
    const resp = await fetch(
  ...
  unmute: async ({ chat, base, authHeaders, controlPlaneEnabled }) => {
    if (!controlPlaneEnabled || !authHeaders) {
      await chat.sendMessage("Control-plane disabled in MVP mode.");
      return;
    }
  ...
  status: async ({ chat, base, authHeaders, controlPlaneEnabled }) => {
    if (!controlPlaneEnabled || !authHeaders) {
      await chat.sendMessage("Control-plane disabled in MVP mode.");
      return;
    }
  ...
  rescan: async ({ chat, parts, base, authHeaders, controlPlaneEnabled }) => {
    if (!controlPlaneEnabled || !authHeaders) {
      await chat.sendMessage("Control-plane disabled in MVP mode.");
      return;
    }
  ...
  if (handler) {
    ...
    const authHeaders = token
      ? {
          ...
          "x-csrf-token": (
            process.env.CONTROL_PLANE_CSRF_TOKEN || token
          ).trim(),
        }
      : undefined;
    ...
    await handler({
      ...
      parts,
      authHeaders,
      controlPlaneEnabled: !!authHeaders,
      base,
      pairingOrchestrator,
      remotePhone,
      sender,
