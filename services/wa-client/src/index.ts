    ...
    readyFallbackTimer = setInterval(async () => {
      ...
      try {
        ...
        if (client.info?.wid) {
          logger.info(
            { pollCount },
            "Detected client.info is populated - triggering ready fallback",
          );
        ...
        if (state === "CONNECTED" && client.info?.wid) {
          logger.info(
            { pollCount, state },
            "Client state is CONNECTED - triggering ready fallback",
          );
        ...
        if (pollCount >= maxPolls) {
          ...
          logger.debug(
            ...
            "Ready fallback: Still waiting for client to be ready",
          );
        }
      } catch (err) {
        logger.debug(
