...
describe('dataKeyProvider', () => {
  ...
  it('uses raw data key and caches materials', async () => {
    const { loadEncryptionMaterials } = await import('../crypto/dataKeyProvider');
    const key = Buffer.from('super-secret-key').toString('base64');

    const first = await loadEncryptionMaterials({ dataKey: key } as any, logger as any);
    const second = await loadEncryptionMaterials({ dataKey: key } as any, logger as any);
  ...
  it('decrypts data key via KMS when configured', async () => {
    ...
    const sendMock = jest.fn().mockResolvedValue({ Plaintext: Buffer.from('kms-secret') });
  ...
  it('throws when no data key sources are configured', async () => {
    ...
    await expect(loadEncryptionMaterials({} as any, logger as any)).rejects.toThrow(
