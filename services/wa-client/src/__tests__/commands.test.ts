...
jest.mock(
  "confusables",
  () => ({ __esModule: true, default: (input: string) => input }),
  { virtual: true },
);
jest.mock(
  "bottleneck",
...
describe("handleAdminCommand", () => {
  ...
  it("issues mute command for group admins", async () => {
    ...
    (mockChat as any).participants = [
      { id: { _serialized: "user-1" }, isAdmin: true, isSuperAdmin: false },
    ];
    ...
    const mockMessage = {
      ...
      getContact: jest
        .fn()
        .mockResolvedValue({ id: { _serialized: "user-1" } }),
  ...
  it("relays rescan confirmation when successful", async () => {
    ...
    expect(sendMessage).toHaveBeenCalledWith(
      "Rescan queued. hash=abc123 job=job-1",
    );
  });
  ...
  it("approves membership overrides from admins", async () => {
    ...
    (mockChat as any).participants = [
      { id: { _serialized: "admin-1" }, isAdmin: true, isSuperAdmin: false },
    ];
    ...
    const mockMessage = {
      ...
      getContact: jest
        .fn()
        .mockResolvedValue({ id: { _serialized: "admin-1" } }),
    } as unknown as Message;

    await handleAdminCommand(
      {
        approveGroupMembershipRequests: approveMembership,
    ...
    expect(approveMembership).toHaveBeenCalledWith("group-456", {
      requesterIds: ["pending-user"],
      sleep: null,
    });
    expect(sendMessage).toHaveBeenCalledWith(
      "Approved membership request for pending-user.",
    );
  });
  ...
  it("summarizes governance events for admins", async () => {
    ...
    await handleAdminCommand(
      {} as Client,
      { ...baseMessage, body: "!scanner consent" },
      undefined,
      {} as unknown as Redis,
    );
    ...
    await handleAdminCommand(
      {} as Client,
      { ...baseMessage, body: "!scanner governance 5" },
      undefined,
      {} as unknown as Redis,
    );

    expect(sendMessage).toHaveBeenCalledWith(
      expect.stringContaining("consent_granted"),
    );
  });
});
...
describe("formatGroupVerdict", () => {
  it("limits reasons and formats output", () => {
    const message = formatGroupVerdict(
      "suspicious",
