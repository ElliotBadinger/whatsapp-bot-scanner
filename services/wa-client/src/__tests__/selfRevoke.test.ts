...
describe("handleSelfMessageRevoke", () => {
  it("skips when session is not ready", async () => {
    ...
    const recordRevocation = jest.fn(
      async () => undefined,
    ) as jest.MockedFunction<RecordRevocationFn>;
  ...
  it("skips status broadcast revocations", async () => {
    const message = createMessage({
      id: {
        _serialized: "false_status@broadcast",
  ...
  it("throws enriched error when chat lookup fails", async () => {
    ...
    await expect(
      handleSelfMessageRevoke(message, {
        snapshot,
        logger: logger as any,
        messageStore,
        recordMetric: jest.fn(),
      ...
      /WhatsApp Web evaluation failed during message_revoke_me:getChat/,
    );
  });
});
