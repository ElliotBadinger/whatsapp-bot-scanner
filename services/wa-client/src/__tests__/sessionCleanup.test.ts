...
describe('resetRemoteSessionArtifacts', () => {
  it('removes stored artifacts and recreates fresh directories', async () => {
    ...
    const tempDir = path.join(dataPath, 'wwebjs_temp_session_default', 'Default');
    ...
    const store = { delete: deleteMock } as RemoteSessionCleanupOptions['store'];
    const logger = {
      warn: jest.fn(),
      info: jest.fn(),
      debug: jest.fn(),
    ...
    const recreatedDefaultDir = path.join(dataPath, 'wwebjs_temp_session_default', 'Default');
