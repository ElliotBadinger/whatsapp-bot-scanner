...
describe('Cache Performance Tests', () => {
  ...
  describe('Cache Lookup Performance', () => {
    test('cache lookup completes in <1ms (hit)', () => {
      ...
      console.log(`   Throughput: ${Math.floor(iterations / (elapsed / 1000)).toLocaleString()} ops/sec`);
  ...
  describe('Cache Memory Efficiency', () => {
    test('cache memory usage scales linearly', () => {
      const testCache = new VerdictCache({ ttlSeconds: 3600, maxKeys: 100000 });
      ...
      for (const count of [1000, 5000, 10000, 25000]) {
        for (let i = measurements[measurements.length - 1]?.entries || 0; i < count; i++) {
      ...
      for (const { entries, memory } of measurements) {
        ...
        console.log(`   ${entries.toLocaleString()} entries: ${(memory / 1024 / 1024).toFixed(2)}MB (${perEntry.toFixed(0)} bytes/entry)`);
      }

      // Verify roughly linear scaling
      const firstRatio = measurements[0].memory / measurements[0].entries;
      const lastRatio = measurements[measurements.length - 1].memory / measurements[measurements.length - 1].entries;
  ...
  describe('Cache Concurrent Access', () => {
    ...
    test('handles mixed concurrent read/write', async () => {
      ...
      console.log(`   Total ops: ${totalOps.toLocaleString()}`);
      console.log(`   Elapsed: ${elapsed.toFixed(2)}ms`);
      console.log(`   Throughput: ${Math.floor(totalOps / (elapsed / 1000)).toLocaleString()} ops/sec`);
