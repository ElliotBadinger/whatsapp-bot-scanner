...
describe('Load Testing', () => {
  describe('Concurrent Scoring Operations', () => {
    ...
    test('handles 1000 sequential scoring calculations in <1s', async () => {
      ...
      console.log(`   Elapsed: ${elapsed.toFixed(2)}ms`);
      console.log(`   Throughput: ${Math.floor(1000 / (elapsed / 1000))} ops/sec`);
  ...
  describe('Concurrent URL Processing', () => {
    ...
    test('handles 50 concurrent extraHeuristics calls in <200ms', async () => {
      ...
      console.log(`   Elapsed: ${elapsed.toFixed(2)}ms`);
      console.log(`   IP literals detected: ${results.filter(r => r.isIpLiteral).length}`);
  ...
  describe('Concurrent Homoglyph Detection', () => {
    test('handles 30 concurrent homoglyph detections in <500ms', async () => {
      ...
      const allDomains = Array.from({ length: 30 }, (_, i) => domains[i % domains.length]);
      ...
      console.log(`   Elapsed: ${elapsed.toFixed(2)}ms`);
      console.log(`   Homoglyphs detected: ${results.filter(r => r.detected).length}`);
  ...
  describe('Cache Performance Under Load', () => {
    ...
    test('handles 1000 concurrent cache writes in <500ms', async () => {
      const writes = Array.from({ length: 1000 }, (_, i) => ({
        hash: `write_hash_${i}`,
        verdict: {
          verdict: (i % 3 === 0 ? 'malicious' : i % 2 === 0 ? 'suspicious' : 'benign') as 'benign' | 'suspicious' | 'malicious',
    ...
    test('handles mixed read/write workload under load', async () => {
      ...
      console.log(`   Elapsed: ${elapsed.toFixed(2)}ms`);
      console.log(`   Throughput: ${Math.floor(operations / (elapsed / 1000))} ops/sec`);
      console.log(`   Hit Rate: ${stats.hitRate}%`);
  ...
  describe('Full Processing Pipeline Load', () => {
    test('simulates 50 URL scan processing in <2s', async () => {
      const urls = Array.from({ length: 50 }, (_, i) => 
        `https://example${i}.com/path/${i}?id=${i}`
      );
      ...
      console.log(`   Verdicts: benign=${validResults.filter(r => r?.verdict.level === 'benign').length}, suspicious=${validResults.filter(r => r?.verdict.level === 'suspicious').length}, malicious=${validResults.filter(r => r?.verdict.level === 'malicious').length}`);
    ...
    test('simulates 100 URL batch processing in <5s', async () => {
      const urls = Array.from({ length: 100 }, (_, i) => 
        `https://test${i}.org/page/${i}?token=${Math.random()}`
      );
      ...
      console.log(`   Elapsed: ${elapsed.toFixed(2)}ms`);
      console.log(`   Throughput: ${Math.floor(100 / (elapsed / 1000))} URLs/sec`);
