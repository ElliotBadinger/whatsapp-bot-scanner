...
describe("error classification helpers", () => {
  ...
  it("classifies undici timeout errors", async () => {
    const { __testables } = await loadIndex();
    const reason = __testables.classifyError({
...
describe("extractUrlscanArtifactCandidates", () => {
  it("returns unique screenshot and dom candidates with defaults", () => {
    ...
    const candidates = jest
      ...
      .__testables.extractUrlscanArtifactCandidates("abc", {
        ...
        task: {
          screenshotURL: "/screens/custom.png",
          domURL: "/dom/custom.json",
        },
        domURL: "/dom/abc.json",
    ...
    expect(
      candidates.every((c: ArtifactCandidate) =>
        new URL(c.url).hostname.endsWith("urlscan.io"),
      ),
    ).toBe(true);
  });
  ...
  it("omits artifact candidates outside the trusted host", () => {
    ...
    expect(
      candidates.every(
        (c: ArtifactCandidate) =>
          !c.url.includes("evil.example") &&
          !c.url.includes("malicious.invalid"),
      ),
    ).toBe(true);
  });
});
...
describe("normalizeUrlscanArtifactCandidate", () => {
  const baseUrl = (
    jest.requireActual("@wbscanner/shared").config.urlscan.baseUrl ||
    "https://urlscan.io"
  ).replace(/\/+$/, "");
