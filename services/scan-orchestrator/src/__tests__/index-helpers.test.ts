...
describe("scan-orchestrator index helpers", () => {
  test("normalizeVerdictReason maps known categories", () => {
    ...
    expect(normalizeVerdictReason("Domain registered 3 days ago (<7)")).toBe(
      "domain_age_lt7",
    );
    expect(normalizeVerdictReason("Domain registered 9 days ago (<14)")).toBe(
      "domain_age_lt14",
    );
    expect(normalizeVerdictReason("Domain registered 20 days ago (<30)")).toBe(
      "domain_age_lt30",
    );
    ...
    expect(
      normalizeVerdictReason("Suspicious homoglyph characters detected"),
    ).toBe("homoglyph_medium");
    ...
    expect(
      normalizeVerdictReason("Redirect leads to mismatched domain/brand"),
  ...
  test("normalizeUrlscanArtifactCandidate validates host and normalizes URLs", () => {
    ...
    const nonString = normalizeUrlscanArtifactCandidate(
      123 as unknown,
      baseUrl,
    );
  ...
  test("buildProviderStates summarizes provider availability", () => {
    ...
    const urlhausState = providerStates.find(
      (state) => state.key === "urlhaus",
    );
