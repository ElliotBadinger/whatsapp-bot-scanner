...
const workingVerbs = /(I[’']?m working|Starting|Reviewing|Investigating|Summarizing)/i;
const noReplies = /(can'?t be interrupted|can'?t.*see replies|won'?t see replies|can'?t pause|can'?t be stopped)/i;
...
export function hasPendingSuggestion(comments) {
  ...
  const suggestions = sorted.filter((comment) => {
    ...
    return author.toLowerCase() === 'charliecreates' && suggestionRegex.test(comment.body || '');
  });
