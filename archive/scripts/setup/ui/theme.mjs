...
function createBox(content, options = {}) {
  ...
  for (let i = 0; i < padding; i++) {
    result.push(
      borderColor(chars.v) + " ".repeat(innerWidth) + borderColor(chars.v),
    );
  }
  ...
  for (const line of lines) {
    const paddedLine =
      " ".repeat(padding) +
      line +
      " ".repeat(Math.max(0, innerWidth - stripAnsi(line).length - padding));
...
function stepList(steps, currentIndex, options = {}) {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    // Step line
    const estimate =
      showEstimate && step.estimate ? estimateStyle(` (${step.estimate})`) : "";
...
function largeCode(code, options = {}) {
  ...
  lines.push(
    COLORS.primaryBold("  ║    ") +
      chalk.bgHex("#003344").hex("#00D9FF").bold(`    ${formattedCode}    `) +
      COLORS.primaryBold("    ║"),
  );
  lines.push(COLORS.primaryBold("  ║                                    ║"));
  lines.push(COLORS.primaryBold("  ╚════════════════════════════════════╝"));
...
function banner(options = {}) {
  ...
  if (compact) {
    return [
      ...
      COLORS.primaryBold("  │") +
        ...
        ICONS.shield +
        " " +
        COLORS.textBold("WhatsApp Bot Scanner") +
        "           " +
        COLORS.primaryBold("│"),
      COLORS.primaryBold("  │") +
        "  " +
        COLORS.muted(`Setup Wizard v${version}`) +
        "                    " +
        COLORS.primaryBold("│"),
      COLORS.primaryBold("  ╰──────────────────────────────────────────╯"),
      "",
  ...
  return [
    ...
    COLORS.primary("        ║") +
      ...
      COLORS.muted("|") +
      "  " +
      COLORS.accent(`Setup v${version}`) +
      "          " +
      COLORS.primary("║"),
    COLORS.primary("        ║") +
      "     " +
      COLORS.muted("Protect your groups from malicious links") +
      "           " +
      COLORS.primary("║"),
    COLORS.primary("        ║") +
      "                                                         " +
...
function completionSummary(options = {}) {
  ...
  const lines = [
    ...
    COLORS.successBold("  ║") +
      ...
      ICONS.sparkle +
      " " +
      COLORS.successBold("Setup Complete!") +
