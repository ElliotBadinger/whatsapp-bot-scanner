...
describe('PairingManager Auto-Detection', () => {
  ...
  describe('monitorForPairingSuccess()', () => {
    it('should initialize monitoring with callbacks', async () => {
      ...
      const result = await pairingManager.monitorForPairingSuccess(successCallback, errorCallback);
    ...
    it('should handle errors during monitoring setup', async () => {
      ...
      pairingManager.startMonitoring = vi.fn().mockRejectedValue(new Error('Docker not available'));

      const result = await pairingManager.monitorForPairingSuccess(successCallback, errorCallback);

      expect(result).toBe(false);
      expect(errorCallback).toHaveBeenCalled();
      expect(mockUi.error).toHaveBeenCalledWith(expect.stringContaining('Failed to start pairing success monitoring'));
    });
  });
  ...
  describe('Log Processing', () => {
    it('should process pairing success events', () => {
      ...
      pairingManager.setupPairingSuccessDetection(successCallback, errorCallback);
      ...
      expect(pairingManager.handlePairingSuccess).toHaveBeenCalledWith(testLine);
    });
    ...
    it('should process rate limit errors', () => {
      ...
      expect(pairingManager.handleRateLimitError).toHaveBeenCalledWith(testLine);
    });
  ...
  describe('Success Handling', () => {
    it('should handle pairing success with session info', () => {
      ...
      const testLine = 'remote_session_saved: session_abc123 successfully established';
      ...
      expect(mockUi.success).toHaveBeenCalledWith(expect.stringContaining('WhatsApp pairing completed successfully'));
  ...
  describe('Error Handling', () => {
    it('should handle pairing errors', () => {
      ...
      expect(mockUi.error).toHaveBeenCalledWith(expect.stringContaining('WhatsApp pairing encountered an error'));
    ...
    it('should handle rate limit errors', () => {
      ...
      expect(mockUi.warn).toHaveBeenCalledWith(expect.stringContaining('Rate limit detected'));
      expect(mockUi.info).toHaveBeenCalledWith(expect.stringContaining('Attempting to recover from rate limit'));
    });
  });
