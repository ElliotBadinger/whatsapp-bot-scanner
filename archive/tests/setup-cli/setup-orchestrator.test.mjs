...
async function cleanupLogs() {
  const target = process.env.SETUP_LOGS_DIR ? path.resolve(process.env.SETUP_LOGS_DIR) : LOGS_DIR;
  try {
    const entries = await fs.readdir(target);
    await Promise.all(
      entries
        .filter(name => name.startsWith('setup-') && (name.endsWith('.json') || name.endsWith('.md')))
...
async function cleanupPreferences() {
  const cacheDir = process.env.SETUP_CACHE_DIR ? path.resolve(process.env.SETUP_CACHE_DIR) : SETUP_DIR;
  try {
...
async function latestTranscript() {
  ...
  const jsonFiles = entries.filter(name => name.startsWith('setup-') && name.endsWith('.json'));
...
describe('setup wizard end-to-end (noninteractive)', () => {
  ...
  it('supports resume checkpoints via quick preflight', async () => {
    ...
    const transcript = JSON.parse(await fs.readFile(transcriptPath, 'utf8'));
    expect(transcript.status).toBe('success');
    expect(transcript.metadata.checkpoints.map(cp => cp.id)).toEqual(['preflight']);
    await fs.rm(tmpDir, { recursive: true, force: true });
  });
  ...
  it('purges caches when requested', async () => {
    ...
    const purgeTranscript = JSON.parse(await fs.readFile(purgeTranscriptPath, 'utf8'));
    expect(purgeTranscript.status).toBe('success');
    expect(purgeTranscript.events.some(event => event.type === 'purge')).toBe(true);
    await fs.rm(tmpDir, { recursive: true, force: true });
  });
});
