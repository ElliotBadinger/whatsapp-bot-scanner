...
const targetFunction = `export function assertEssentialConfig(serviceName: string): void {
  ...
  if (config.phishtank?.enabled && !config.phishtank.appKey) missing.push("PHISHTANK_APP_KEY");
  if (config.urlscan?.enabled && !config.urlscan.apiKey) missing.push("URLSCAN_API_KEY");
...
if (code.includes(targetFunction)) {
    ...
    console.log("Reverted config.ts successfully!");
} else {
    console.log("Could not find the target function to revert.");
}
