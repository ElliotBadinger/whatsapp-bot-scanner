...
} = require("./packages/shared/dist/reputation/certificate-intelligence");
...
async function testCertificateValidation() {
  ...
  try {
    const result1 = await certificateIntelligence("google.com", {
      timeoutMs: 3000,
  ...
  try {
    const result2 = await certificateIntelligence("self-signed.badssl.com", {
      timeoutMs: 3000,
      ctCheckEnabled: false,
    });
  ...
  try {
    const result3 = await certificateIntelligence("wrong.host.badssl.com", {
      timeoutMs: 3000,
      ctCheckEnabled: false,
    });
  ...
  try {
    const result4 = await certificateIntelligence("expired.badssl.com", {
      timeoutMs: 3000,
      ctCheckEnabled: false,
    });
